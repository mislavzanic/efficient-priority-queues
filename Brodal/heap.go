...
func (bh *BrodalHeap) Meld(other *BrodalHeap) {
		...
		if maxTree.RootRank() == 0 {
			...
			for _, tree := range others {
				if tree != minTree.tree1 {
					...
					// println("ajmo rang", maxTree.id, minTree.tree1.RootRank())
	...
	// println()
	// for e := bh.t1s.tree1.root.children.Front(); e != nil; e = e.Next() {
	// 	println("rankloop", e.Value.(*node).rank)
	// }
	// println("------------")
	// if bh.tree2 != nil {
...
func (bh *BrodalHeap) updateWSet(bad *node) {
	...
	for _, act := range acts {
		if act.op == Increase {
			if bh.t1s.t1GuideW.boundArray[bad.rank].fst != 0 {
...
func (bh *BrodalHeap) derankAndInsertRootChild(removeRoot *tree, insertRoot *tree, child *node) {
	...
	// implementirati force decrease u guide-u
...
func (bh *BrodalHeap) updateLowRank(tree *tree, node *node, insert bool) {
	...
	println("novi", tree.root.numOfChildren[node.rank], node.rank)
	if len(tree.root.numOfChildren) > 1 {
		println("drugi", tree.root.numOfChildren[1], "id", tree.id)
	}
	...
	for _, act := range response {
		if insert {
			if act.op == Increase {
				println("adding", node.rank)
				tree.addRootChild(node)
				bh.mbyAddViolation(node)
			} else {
				if tree.root.numOfChildren[act.index] != 7 {
					println("guide...", tree.upperBoundGuide.boundArray[act.index].fst, tree.root.numOfChildren[act.index], act.index)
...
func (bh *BrodalHeap) incTreeRank(tree *tree, node1 *node, node2 *node) {
	...
	if tree.id == 1 {
		...
		bh.t1s.t1GuideW.expand(tree.RootRank() + 1)
	}
